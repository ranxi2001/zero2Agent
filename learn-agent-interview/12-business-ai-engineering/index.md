...
具体落地时，AI Coding 可用提交耗时、测试通过率、缺陷率和返工率衡量；Agent 要把流程拆成可观测步骤，比较任务完成时长、成功率与人工介入率。文章质量规则应来自编辑规范、历史问题和用户反馈，先人工标注一致性，再离线评测并抽样复核，灰度后持续监控误报、漏报及阅读、转化指标。
...
图文混合审核先分别做图片检测、OCR和文本切片，再做跨模态对齐，判断文字是在描述、转述还是支持图片中的风险；高风险样本保留证据并人工复核。内容生产还需检查提示注入、隐私或版权素材、敏感信息及平台规则，发布前后分别拦截、降级或复核，并用误报、漏报和申诉率回归。
...
Workflow可定义统一状态对象：采集输入URL与授权结果，输出正文和证据；理解输入原文，输出结构化Brief；生成输入Brief与渠道约束，输出候选稿；验证输入候选稿与证据，输出通过、修复或人工审批。Multi-Agent由编排器传递版本化消息和共享状态，各Agent只读所需字段，失败可从最近Checkpoint重试或回滚。
...
> 来源：[9.11 Boss直聘--一面](https://www.nowcoder.com/feed/main/detail/51ff89e97d4949bd9db8e12a605b7aa9)；本轮追问：如何设计营销会场秒杀系统？（[字节广告团队agent面经（一二面）](https://www.nowcoder.com/discuss/930870582843805696)）
...
> 来源：[卓驭 正式批 一面 数据算法与测评工程师 已挂](https://www.nowcoder.com/feed/main/detail/46556042061840eca6af69727e72909c)；[蔚来——大模型算法岗（智能座舱/自动驾驶）实习一面](https://www.nowcoder.com/discuss/930755708008558592)
...
## Q：如何生成既有吸引力又不重复的推荐理由？

> 来源：[小红书多模态秋招二面](https://www.nowcoder.com/discuss/930755993066041344)

**新手答**：“可以结合用户兴趣和笔记内容生成个性化文案，并用相似度检测和模板轮换减少重复。”

**高手答**：

把系统拆成“证据抽取、候选生成、去重校验、质量评估”四层：先从笔记标题、主题、作者特征和用户行为中抽取可验证亮点，要求生成结果引用这些证据，避免泛化夸赞或编造内容；再用模板、受控解码或多个候选生成不同表达。去重不能只做字符串比较，还应结合 n-gram、向量相似度和用户近期曝光历史，设置全局、用户级与时间窗口阈值。对长度、敏感词、事实一致性、语气和可读性做规则及模型校验，失败则回退到安全模板。离线评估覆盖点击、阅读、隐藏和重复率，线上用分桶实验观察长期指标，并限制延迟、成本和曝光频次；多样性过强可能损害相关性，因此要按场景调节权重。

**差距在哪**：浅层回答停留在“个性化加去重”，深入回答会明确证据约束、分层去重、失败回退、离线与线上评估及相关性和多样性的权衡。

---

## Q：如何设计一个具备情绪识别与情感响应能力的 Agent？

> 来源：[蔚来——大模型算法岗（智能座舱/自动驾驶）实习一面](https://www.nowcoder.com/discuss/930755708008558592)

**新手答**：“先通过文本、语音或表情识别用户情绪，再结合情绪生成更有同理心的回复。”

**高手答**：

情感交互 Agent 通常分为感知、状态管理、策略决策和响应生成四层：先从文本、语音或图像提取情绪类别、强度与置信度，再结合对话历史维护短期情绪状态；策略层决定安抚、澄清、拒答或转人工，生成层控制语气、内容和行动。情绪识别只是概率判断，不能把低置信度结果当事实，应允许用户纠正，并对自伤、暴力、医疗等高风险信号设置安全流程。工程上要最小化采集敏感数据、限制记忆范围、记录决策依据并支持审计。验证应覆盖分类准确性、误判伤害、回复一致性、用户满意度和人工接管率，同时用对抗样本和真实脱敏会话回放测试。更强的共情可能提高体验，但也会增加幻觉、过度拟人和隐私风险。

**差距在哪**：浅层只描述“识别情绪后生成安慰话术”，深入回答还应覆盖不确定性、安全升级、隐私治理、评估指标与误判代价。
...
**新手答**：“视频链路通常是上传到对象存储，经过异步转码生成多种清晰度，再通过 CDN 分发播放。”

**高手答**：

完整链路可拆为上传、处理、存储、分发和观测：客户端先申请上传凭证，将文件直传对象存储；服务端记录媒体元数据并投递转码任务，Worker 做病毒或格式检查、探测时长与编码，生成多码率文件、缩略图和播放清单，再写入持久化存储。播放端通过鉴权 URL 从 CDN 获取清单和分片，回源读取对象存储。设计重点是任务幂等、断点续传、大小与格式限制、租户隔离、加密和权限控制；转码失败要可重试、可人工补偿且避免重复扣费。用链路追踪、任务状态机、抽样播放和清晰度/首帧指标验证。预转码稳定但成本高，按需转码节省资源却增加首播延迟。
