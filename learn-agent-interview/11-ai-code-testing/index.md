...
> 来源：快手测试开发实习一面（2026-08-12）；[美团AI Agent一面](https://www.nowcoder.com/feed/main/detail/50bcdc47e7754aa7be59b6318fea514b)
...
## Q：如何评估并保证 AI Code Reviewer 的审查正确性？

> 来源：[字节跳动Agent开发1面凉经](https://www.nowcoder.com/discuss/929406267141914624)

**新手答**：“通过人工抽样、测试用例和代码扫描结果对比，持续检查 AI 的误报和漏报，并在高风险修改上保留人工审核。”

**高手答**：

AI Code Reviewer 的角色应是发现风险、解释证据并辅助决策，而不是默认替代合并审批。先建立覆盖安全、正确性、性能和可维护性的标注集，按问题严重级别统计 precision、recall、漏报率和误报率；再用历史缺陷、人工构造缺陷和变异测试验证是否能发现真实问题。生产中可先影子运行，与人工审查结果做盲评，按仓库和语言持续回归，并记录提示词、模型、规则与输入版本。高风险结论必须给出代码位置、推理依据和可复现检查，低置信度只建议不阻断。还要限制敏感代码外泄，设置人工复核、灰度和回滚机制。核心权衡是覆盖率、误报成本与审查延迟，不能只看“发现数量”。

**差距在哪**：浅层只会说抽样和人工复核，深入回答还应覆盖标注集、误报漏报指标、变异测试、影子运行、证据链与高风险兜底，这是面试官考察的评估闭环。

---

## Q：HumanEval、MBPP、APPS 的数据集结构与评估方式是什么？

> 来源：[字节算法（Agent）凉经](https://www.nowcoder.com/feed/main/detail/6d80836574e647d39eb9842dc4131ace)

**新手答**：“HumanEval、MBPP 和 APPS 都是代码生成评测集，通常让模型生成程序，再运行测试用例判断是否通过。”

**高手答**：

三者都以“题目输入—生成代码—执行测试”为核心，但粒度不同。HumanEval 通常提供函数签名、题意、入口名和测试约束，重点是补全函数；其官方仓库包含任务表示与 functional-correctness 评测脚本（[OpenAI HumanEval Repository](https://github.com/openai/human-eval)）。MBPP 以自然语言编程题、参考实现和测试为主，评测生成代码是否通过题目测试（[Google Research MBPP Dataset](https://github.com/google-research/google-research/tree/master/mbpp)）。APPS 更接近完整竞赛题，包含题面、输入输出说明、代码上下文和测试数据，覆盖更复杂的程序生成场景（[APPS Repository](https://github.com/hendrycks/apps)）。常见指标是 pass@1 或多次采样后的 pass@k；必须在隔离沙箱中限制网络、文件、资源和执行时间，并区分编译失败、超时、运行错误与断言失败。评测还要固定提示模板、语言版本和随机种子，避免数据泄漏及测试不充分造成虚高分。

**差距在哪**：浅层只知道“生成代码后跑测试”，深入回答要区分三套数据结构、执行指标、采样口径、沙箱安全，以及测试覆盖不足和数据泄漏带来的评测偏差。
