...
1. **冷启动开销**：每次连接都要走 `initialize → tools/list`，工具描述全量注入上下文，占用 token 预算。100 个工具 × 200 token/工具 = 20K token 还没开始干活就花了
2. **无状态设计的代价**：工具之间数据传递只能靠上下文中转——工具 A 的输出要传给工具 B，必须先回到模型再传出去，不能工具间直接通信
3. **安全模型粗糙**：OAuth2.1 刚引入，权限粒度不够细（只能控制“能不能调某个工具”，不能控制“能用什么参数调”）
4. **生态碎片化**：社区 MCP Server 质量参差不齐，缺乏认证和审核机制，恶意 Server 是安全隐患
