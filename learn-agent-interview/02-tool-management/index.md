...
> 来源：腾讯 Agent 岗终面；[去哪儿 AI 全栈 AI 面](https://www.nowcoder.com/feed/main/detail/9cf516b3c2404100baeac52564e40709)；本轮追问：模型生成工具参数出错如何处理？（[千问AI研发一面凉经](https://www.nowcoder.com/feed/main/detail/ebf0ec03e5ee4fa8a140cc8d247b1e20)）
...
Agent 是运行时的智能体或编排层，负责理解目标、维护上下文、规划并调用 Skill 或 MCP 工具；Skill 是面向任务的流程与约束，MCP 是连接外部工具和资源的标准协议。三者分别对应执行主体、能力封装和通信接口。
...
> 来源：Agent 开发面试 30 题；[携程 AI 应用开发一面（已oc）](https://www.nowcoder.com/feed/main/detail/2b1c35eace4a4bb3bd88ee669f163793)
...
框架接入时由 MCP Client 负责初始化、能力协商和工具发现，再将工具 Schema 适配为框架的统一 Tool 注册表；执行时按工具标识路由回 Client 调用，并统一处理超时、取消、错误和结果格式。运行中的编排应使用已缓存的能力快照，工具变更在下一轮生效。
...
复合指令先拆成带依赖关系的子任务，例如先执行打开空调，再设置温度；无依赖的工具可并行调用。每一步都要继承会话状态，校验工具返回结果，失败时停止后续动作或补偿重试；涉及车控等有风险操作还应确认、鉴权，并保证调用幂等。
...
> 来源：淘天 AI Agent 一面；本轮追问：MCP 工具具有动态发现能力，如何保证编排执行时能力稳定？（[字节 aime 一面 9.10](https://www.nowcoder.com/feed/main/detail/ed5e9d17f26e489da94afbf1241b885e)）
...
编排启动时应获取并固定工具能力快照，记录工具版本、Schema 和权限，执行期间不因动态刷新改变参数契约；注册中心需做健康检查和兼容校验。调用失败时按超时、重试、熔断及候选工具降级处理，变更只影响后续编排并保留审计记录。
...
> 来源：拓竹 AI Agent 算法一面（2026-08-18）；本轮追问：你的治理SOP是什么形式的？里面有哪些内容？（[本轮追问](https://www.nowcoder.com/feed/main/detail/595a0cb450cf45e9a47a0d32084b9099)）；本轮追问：这类 Skill 和 MCP 有哪些安全规范？如何保证安全并避免漏洞？（[快手AI全栈三面](https://www.nowcoder.com/discuss/929747739645136896)）
...
Skill 和 MCP 应遵循最小权限、服务端白名单、版本或签名校验及密钥隔离；对参数做 schema 校验和资源范围限制，防 SSRF、路径穿越与注入，工具运行置于沙箱，高风险写操作需人工审批，并支持审计、撤权和快速回滚。
...
> 来源：阿里控股 Agent Infra 暑期一面【[未知公司 Agent 二面](https://www.nowcoder.com/feed/main/detail/16675d793c0c42e8a6b46d42fb561561)追问：如何判断 CLI/MCP 是否 AI-Friendly】；本轮追问：如何判断工具是否可信，并允许它在 Sandbox 中直接执行？（[字节 aime 一面 9.10](https://www.nowcoder.com/feed/main/detail/ed5e9d17f26e489da94afbf1241b885e)）
...
可信工具应具备可追溯来源、固定版本或内容哈希、最小权限声明，并通过静态检查、隔离动态测试和恶意输入回归。允许 Sandbox 直执行前，限制文件系统、网络、凭据和资源配额，默认无副作用；高风险操作需审批，保留审计、回滚与熔断开关。
...
## Q：Agent 中 Skill 与 Tool 的职责边界和选型标准是什么？

> 来源：[字节数据中台Agent全栈面经](https://www.nowcoder.com/feed/main/detail/fe77496948f74b4091ab4bb6e8156c8b)

**新手答**：“Skill 更适合描述可复用的流程和知识，Tool 更适合执行具体动作或调用外部系统。”

**高手答**：

Skill 是可组合、可复用的行为规范或流程，通常包含目标、步骤、提示模板、领域知识和判断规则，本身不一定直接产生外部副作用；Tool 是模型可调用的明确接口，负责查询数据、写文件、发请求或执行计算，并应定义参数、返回值、权限和错误。选型时看能力是否需要实时数据、确定性执行和外部副作用：偏知识与流程编排用 Skill，必须访问系统或完成动作则用 Tool。工程上要限制权限、校验参数、设置超时，处理重试、幂等和部分失败；用离线用例、工具调用日志、成功率与人工抽检验证效果。Skill 过大难维护，Tool 过多会增加选择歧义和攻击面。

**差距在哪**：浅层回答只给出“流程对 Skill、动作对 Tool”的定义，面试官还会考察接口边界、权限与副作用、失败重试、幂等性及如何通过调用评测验证选型。

---

## Q：工具调用结果如何在 Agent 工作流中直接传递？什么情况下需要 LLM 参与中转？

> 来源：[字节 aime 一面 9.10](https://www.nowcoder.com/feed/main/detail/ed5e9d17f26e489da94afbf1241b885e)

**新手答**：“如果工具输出格式明确且下一个工具能直接消费，就应在工作流中传递，只有需要理解或决策时才让 LLM 介入。”

**高手答**：

工具直传适用于确定性的工作流：上游返回带版本的结构化结果，下游按 schema 读取字段，编排器负责校验、路由、重试和错误传播。这样可减少延迟、成本和模型幻觉，也更容易审计与重放。LLM 介入的场景是需要语义判断、规划下一步、从非结构化文本提取信息，或在多个工具能力间做不确定选择；但其输出必须经过 schema、权限和业务规则校验，不能直接成为高风险写操作。工程上要限制载荷大小，避免把密钥和无关数据传递给模型，保留来源、版本和关联 ID，并设计超时、幂等、部分失败与人工接管。验证可用固定工作流回放、契约测试和故障注入；确定性优先，灵活性只在有收益处引入。

**差距在哪**：浅层只把 LLM 当作工具链中转，深入回答会区分确定性数据流与语义决策，进一步覆盖 schema、权限、幂等、失败恢复、可审计性和成本延迟权衡。
