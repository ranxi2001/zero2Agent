...
> 来源：抖音基础架构 Agent 一面；本轮追问：用户消息、系统提示词、Memory 等信息如何组装并发送给模型？（[字节 aime 一面 9.10](https://www.nowcoder.com/feed/main/detail/ed5e9d17f26e489da94afbf1241b885e)）
...
发送时将内容组装为模型 API 的结构化消息：稳定的 system、经过筛选和压缩的 Memory、当前任务与用户消息，工具定义按需附加，避免把不可信内容混入高优先级指令。组装前做脱敏、权限过滤和 token 预算，发送后记录模板版本与上下文摘要，便于复现。
...
> 来源：抖音基础架构 Agent 一面 【小红书AI应用开发同题：Skills了解+如何管理各个Skills】【CVTE AI应用工程师一面追问：怎么理解 Skill？能解决什么问题？怎么写 MCP？】；本轮追问：Agent 没有遵循 Skill 中的限制和描述时，你是如何解决的？（[字节 AI 全栈一面（飞书）](https://www.nowcoder.com/discuss/931555466247700480)）；本轮追问：Skill 有哪几种格式？（[小米--后端--二面](https://www.nowcoder.com/discuss/929769764719775744)）
...
若模型不遵守 Skill，先检查是否被截断、未正确触发或与更高优先级指令冲突；将不可违背的规则下沉到系统/开发者指令、工具权限和代码校验，并用结构化输出、执行后测试与失败重试验证。Skill 可用独立 Markdown（含元数据）、YAML/JSON 等结构化配置，或项目规则文件内联，具体以框架加载器支持为准。
...
内部平台应将 Skill 作为可校验、可版本化的制品，支持上传审核、下载、发布、回滚和权限控制；本地 Runtime 负责按版本加载，在沙箱中执行并隔离超时、错误和资源。多个 MCP 工具可预先编排为带输入输出 Schema 的有向流程，声明依赖、重试、超时和幂等策略，运行时按拓扑顺序执行并记录结果。
...
典型 Skill 以目录为边界，核心是 SKILL.md：用元数据（如名称、用途、触发条件）描述何时加载，再按需引用正文中的流程、约束和示例；配套可放 scripts、references、assets 等资源。加载时先读摘要，执行任务再按需读取资源，避免一次性塞满上下文。
...
> 来源：腾讯 Agent 应用开发一面；本轮追问：请写一个 prompt。（[本轮追问](https://www.nowcoder.com/feed/main/detail/f9e65706c3274fac86b52f27a70bf902)）；本轮追问：从文档中“提取什么内容”，也是由 Prompt 决定吗？（[度小满一面](https://www.nowcoder.com/feed/main/detail/41baab8c631647568203ebfaf3898574)）
...
是，但 Prompt 只决定抽取目标、字段定义、范围、格式和缺失值处理，不能替代文档解析与检索。应给出字段 Schema、判定规则、原文证据要求和无法确定时的返回方式，并通过带标注样本评估漏抽、误抽和幻觉；长文还要配合分块、召回和后处理校验。
...
> 来源：淘宝闪购 AI应用研发 一面；[Agent 开发面经](https://www.nowcoder.com/discuss/930572525178748928)
...
> 来源：视频面经汇总；[快手 AI 全栈一面](https://www.nowcoder.com/feed/main/detail/a30242712e8d456c839ff4223470f491)；本轮追问：文章质量检测 Skill 包含哪些检查项？（[携程 AI 应用开发二面（已oc）](https://www.nowcoder.com/feed/main/detail/781cfc04e3bc4697acf0a5c913543a28)）
...
文章质量检测还应覆盖：事实准确性与引用可追溯、结构完整性、段落和标题层级、语病与可读性、重复及过时内容、关键词与读者意图匹配，以及图片、链接、代码示例是否有效。
...
Skill 是可复用的任务说明、步骤、约束与校验规则集合。粒度以单一职责、可独立触发和可独立评测为准，过大难复用、过小会增加编排成本。质量看目标清晰、边界完整、依赖明确、输出可验证；用覆盖正常与失败场景的评测集，对比正确率、完成度和冲突率，结合线上失败样本迭代。
...
> 来源：[电商 Agent 三面](https://www.nowcoder.com/feed/main/detail/b6b453976c2d4e43a872054d695c2fe2)【电商库存二面（2026-08-17）】；[度小满一面](https://www.nowcoder.com/feed/main/detail/41baab8c631647568203ebfaf3898574)；[斑头雁（Agent实习生面试一面）](https://www.nowcoder.com/discuss/930139676764102656)
...
> 来源：[阿里边缘 BU 一面](https://www.nowcoder.com/feed/main/detail/bdebbb6088b6405e9eb2bd2c345acb6e)；本轮追问：这个 AI 伴看 Agent 是怎样进行首轮加载的？是用户开始提问时才加载，还是以其他方式加载？（[飞书深诺（全栈AI应用开发方向）](https://www.nowcoder.com/feed/main/detail/46a9336c7ead4586bae34e521d4f61d0)）；[斑头雁（Agent实习生面试一面）](https://www.nowcoder.com/discuss/930139676764102656)
...
首轮不应等用户提问才加载：会话或作品打开时先异步拉取 Skill 索引、版本和权限信息，并预加载高概率使用的摘要或路由配置；真正执行某类任务时再按需获取正文和依赖。首轮加载失败可使用已校验缓存，但要标记版本和过期状态。
...
> 来源：最有料 AI 实习生面经；本轮追问：给你一个 PRD，你会怎么和 AI 进行协同开发？（[本轮追问](https://www.nowcoder.com/feed/main/detail/d0be5ebcc6a0480d8975498fba408250)）；本轮追问：如何设计一个“天气与穿搭”的 Skill/工具？（[度小满一面](https://www.nowcoder.com/feed/main/detail/41baab8c631647568203ebfaf3898574)）
...
天气与穿搭 Skill 可拆为天气查询工具和穿搭决策逻辑：先确认城市、时间、单位及用户偏好，调用天气 API 获取温度、体感温度、降水、风力和预警；工具失败时明确提示并避免臆测。再依据温度、降水、场景生成分层建议，并标注数据时间与不确定性。
...
## Q：如何让大模型从混乱的非结构化文档中稳定提取结构化 JSON？

> 来源：[度小满一面](https://www.nowcoder.com/feed/main/detail/41baab8c631647568203ebfaf3898574)

**新手答**：“可以通过清晰的提示词、固定 JSON 格式和示例，让模型从文档中提取字段。”

**高手答**：

先定义字段契约：字段名、类型、是否必填、枚举范围、缺失值和歧义处理规则，并要求模型只输出符合 Schema 的 JSON。对混乱文档可先做分段、版面恢复、表格解析和字段候选定位，再让模型抽取；长文档采用分块抽取与汇总，避免上下文污染。生产链路应使用结构化输出或 JSON Schema 校验，解析失败自动重试或转人工，不能只依赖提示词。对金额、日期、编号等关键字段进行正则、字典和业务规则复核，保留原文证据与置信度。用标注样本评估字段级准确率、召回率、格式通过率及拒答率，并覆盖缺失、冲突和恶意指令。强约束更稳定但可能增加拒答和成本，宽松抽取则需要更强的后处理。

**差距在哪**：浅层只会写提示词和 JSON 示例，深入回答应覆盖文档预处理、Schema 校验、失败重试、证据留存、业务复核与可量化评估。
