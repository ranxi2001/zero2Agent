...
> 来源：腾讯 Agent 岗终面【[百度 - Agent 研发岗（架构方向）](https://www.nowcoder.com/discuss/926273622006665216)追问：当前 Agent 系统面临的最大工程挑战是什么（上下文、工具、规划）？】；[百度Agent二面，不看简历不问八股](https://www.nowcoder.com/feed/main/detail/c7f00d0e48aa4017911b46ed928d15f3)
...
> 来源：腾讯大模型应用开发二面；本轮追问：除成功率和耗时外，还应关注哪些指标？（[字节 aime 一面 9.10](https://www.nowcoder.com/feed/main/detail/ed5e9d17f26e489da94afbf1241b885e)）
...
> 来源：Agent 开发面试 30 题；本轮追问：线上用户反馈“没有帮助”，主要是哪些场景没有覆盖，原因是什么？（[淘天AI应用开发二面](https://www.nowcoder.com/feed/main/detail/d5d1f688dae5496abbce783aa28d6731)）
...
“没有帮助”还要建立意图覆盖矩阵：按用户任务、行业/实体、语言表达和工具依赖统计请求量、成功率与低评分率，识别未覆盖或覆盖不足的长尾场景。对典型样本标注缺失意图、知识、工具能力或结果解释，并用回放集验证补充后的覆盖率和质量。
...
> 来源：蚂蚁集团一面；[本轮来源](https://www.nowcoder.com/feed/main/detail/022ab580ee68432daf536a939e86aefa)；本轮追问：你看过 DeepSeek 最近发布的相关 Agent 产品吗？（[本轮追问](https://www.nowcoder.com/feed/main/detail/bfd43b5c66784add8fbf5893c164697a)）；[9.15 小红书 AI应用研发 一面](https://www.nowcoder.com/discuss/929475914579136512)
...
质量改进可做成自动闭环：先按固定 Rubric 分维度评分，再让系统针对低分项重写或调整提示词，生成前后版本并做成对比较；结构、事实、引用和格式仍用确定性规则门禁，语义评分需固定 Judge、保留回归集，并以人工抽样校准，防止自动优化迎合评分器。
...
> 来源：淘天 AI Agent 一面；本轮追问：评价 Agent 时，应该关注中间过程，还是最终任务结果？（[9.14小红书 PE（产品工程师/全栈方向--实习）二面 (流程泡到9.21挂)](https://www.nowcoder.com/discuss/929891805049421824)）
...
记忆可分为短期上下文、情节记忆和经验证的语义/程序记忆；写入前做脱敏、去重、冲突检测并保留来源、时间、置信度和适用范围。冲突时按新鲜度、证据强度和权限裁决，无法裁决则并存并在检索时提示；自我改进只生成候选，经回放评测和灰度后更新，失败可回滚。
...
除自建领域集外，可参考 BFCL 类工具调用集测参数和调用正确性，用对抗或越界集测拒识与安全，用 OOD 和时间切片集测泛化与漂移；这些只能作补充，最终仍需领域专家抽检路由意图、Skill 边界和端到端结果。
...
**新手答**：“先检查数据查询和计算链路是否正确，再判断模型是否在单位、筛选、聚合或数值推理上出错。”

**高手答**：

应把结果拆成可审计链路：问题解析、指标与维度映射、权限和时间范围、SQL 或工具调用、原始返回、聚合计算、模型生成与最终展示。先用同一问题执行黄金 SQL 或人工基准，核对数据源版本、字段口径、空值、重复行、时区、单位、精度和权限过滤；若原始结果已错，优先定位元数据、连接器、SQL 生成或执行层。若原始结果正确而答案错，再构造最小数值样本，测试加减、同比、排序、分组、单位换算和多轮上下文，区分解析错误与模型推理错误。生产上应保留 trace、SQL、输入快照和中间结果，加入结果校验、可复算性检查与拒答机制，并用分层回归集量化两类错误率。

**差距在哪**：浅层回答只停留在“查数据或查模型”，深入回答要建立可回放的分层链路、最小对照实验、错误归因指标和安全兜底。

---

## Q：如何搭建微调任务的效果评估体系，并量化模型优化收益？

> 来源：[滴滴一些面经合集（算法）](https://www.nowcoder.com/feed/main/detail/37cae17c5f2a49ee81375721f53bbf9b)

**新手答**：“建立覆盖准确性、稳定性、泛化能力和业务价值的测试集，对比微调前后的指标与典型案例。”

**高手答**：

评估体系应先固定任务定义、数据切分和验收标准，避免训练集泄漏或只挑成功案例。数据层覆盖常见、长尾、边界、对抗、不同长度和不同业务分布，并保留独立回归集；指标层同时看任务正确率、格式或工具调用成功率、事实一致性、拒答质量、延迟、成本与安全风险。可用自动规则、程序化校验、模型裁判和人工盲评结合，但要抽样复核裁判偏差，并报告置信区间或分桶结果。收益量化应与基线、同预算方案和线上对照比较，区分平均提升、长尾改善和回归项；记录随机种子、数据版本、超参及推理配置，做多次运行和显著性检验。微调后还要验证灾难性遗忘、提示变化下的鲁棒性及发布后的监控，不能把单一分数当作生产收益。

**差距在哪**：浅层回答只罗列准确率等指标，深入回答会设计分层数据、可靠评测、基线对照、统计不确定性及线上成本与风险验证。
...
**新手答**：“可以用清晰度、构图、色彩和主体突出程度等指标训练一个分类或回归模型来评估封面图质量。”

**高手答**：

先定义模型角色：它应预测封面对目标场景的可用性，而不只是审美分数。建立覆盖清晰度、曝光、构图、主体突出、文字可读性、相关性和安全风险的标注体系，采用多标注者评分并记录场景、设备和分歧；同时保留点击、停留或转化等结果作为弱监督，但避免把热门内容直接当成高画质。模型可用视觉编码器加多任务分类/回归头，结合裁剪、压缩、缩放和文字遮挡增强，输出总分、分项分数与置信度。按时间、作者和主题切分数据，评估相关性、排序质量、校准、不同设备及长尾场景表现，并做人工抽检和线上 A/B。要防止模板、作者或曝光数据泄漏；单一总分易掩盖缺陷，分项解释更可运营，但标注成本和推理延迟也更高。

**差距在哪**：浅层回答只列审美指标和模型类型，深入回答会处理标签定义、弱监督偏差、数据泄漏、分项解释、校准及线上验证。

---

## Q：故障诊断系统如何降低误报率并平衡漏报率？

> 来源：[蔚来——大模型算法岗（智能座舱/自动驾驶）实习一面](https://www.nowcoder.com/discuss/930755708008558592)

**新手答**：“通过提高告警阈值、过滤短时异常并结合人工复核，可以减少误报，但要同时监控漏报。”

**高手答**：

先定义误报与漏报的业务代价、告警对象和统计口径，不能只看一个准确率。数据侧要处理标签噪声、类别不平衡、传感器漂移和故障发生前的时间窗口；模型侧可采用分层诊断、时间窗口与滞回、置信度校准、代价敏感阈值和“不确定则转人工/延迟确认”。对同一根因的多个症状应做告警聚合，避免重复报警，但不能用简单去重掩盖真实并发故障。验证应按设备、工况、故障类型和时间切分，报告混淆矩阵、精确率、召回率、提前量、每设备告警量及未见故障表现，并在影子模式和灰度阶段监测漂移。阈值应按风险分级：安全关键故障宁可保留较高召回并快速升级，低风险故障则可压低误报；上线后还需闭环复盘人工处置结果。

**差距在哪**：浅层回答只会调高阈值；面试官更关注误漏报成本、时间与设备分层、告警聚合、校准、未知故障、灰度验证和上线后的反馈闭环。
...
**新手答**：“可以通过显著性检验、置信区间和多次重复实验，观察效果是否稳定且明显超过随机波动。”
