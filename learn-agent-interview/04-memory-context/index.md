...
置信度不能只看模型自报，可由意图分类概率、必填参数完整性、检索证据一致性、工具校验结果和历史成功率共同约束；关键字段缺失、候选结果接近、证据冲突或涉及支付删除等不可逆操作时必须二次确认。确认前展示将采用的参数，确认后再执行。
...
> 来源：后端 AI 八股 / Memory 系统；[滴滴一些面经合集（算法）](https://www.nowcoder.com/feed/main/detail/37cae17c5f2a49ee81375721f53bbf9b)
...
> 来源：后端 AI 八股 / Memory 系统；[度小满一面](https://www.nowcoder.com/feed/main/detail/41baab8c631647568203ebfaf3898574)
...
> 来源：后端 AI 八股 / Memory 系统；[阿里云 ai应用开发 一面](https://www.nowcoder.com/feed/main/detail/7e27cf4dedb142d9b643471ba31276ed)
...
> 来源：Agent 开发面试 30 题；[滴滴一些面经合集（算法）](https://www.nowcoder.com/feed/main/detail/37cae17c5f2a49ee81375721f53bbf9b)
...
多 Agent 场景应隔离各自私有上下文，只向共享区写入结构化结论、证据指针和待办，避免互相转发完整对话。为每个 Agent 和协作轮次设 Token、深度与时间预算；将详细推理放在 trace，不直接注入后续 prompt，并用去重、摘要、循环检测和阶段性汇总控制膨胀。
...
动态加载是按会话和当前剧情进度分层取数：进入作品时预加载全局剧情摘要、人物关系和章节索引，用户切到某集后仅检索该集及相关片段，命中后再按需补充原文，并缓存已加载结果。这样避免每轮携带全量剧本；节省比例应通过埋点对比压缩前后输入 Token，按未加载的历史文本占比估算。
...
> 来源：蚂蚁集团 Agent 开发一面 / [OPPO IT 开发一面](https://www.nowcoder.com/discuss/923561467092160512)【[拼多多 - Agent 开发岗（工程化 + 数据库）](https://www.nowcoder.com/discuss/926273867092430848)追问：大规模代码场景的上下文维护？】；本轮追问：模型的上下文里面具体包含哪些内容？（[斑头雁（Agent实习生面试一面）](https://www.nowcoder.com/discuss/930139676764102656)）
...
> 来源：淘天 AI Agent 一面；本轮追问：如果用户问“这个灯亮了什么意思”，Agent怎么诊断？（[蔚来——大模型算法岗（智能座舱/自动驾驶）实习一面](https://www.nowcoder.com/discuss/930755708008558592)）
...
“这个灯”应先结合照片或图标、车型年款、亮灯颜色和是否闪烁，匹配车辆告警码，再读取车速、温度等实时状态与用户手册；无法消歧就选择式追问。若涉及制动、机油压力或持续闪烁，应先提示安全停车并建议人工检修，不能仅凭图像下结论。
...
> 来源：高德 AI 应用开发实习一面；本轮追问：对于像 ChatGPT 这种对话 Agent，如何设计会话表和消息表的字段？（[本轮追问](https://www.nowcoder.com/discuss/926917172448759808)）；[本轮来源](https://www.nowcoder.com/feed/main/detail/2f4e4cde4e524a16aae5f55a89c49273)；本轮追问：Agent 记忆在什么情况下增删改查（[乐言科技一面](https://www.nowcoder.com/feed/main/detail/70faa87a0e1046a68c864a2a7c4c6789)）
...
记忆的增加应限于经确认的偏好、承诺和长期事实；查询发生在生成前，按当前任务相关性检索；用户明确纠正或事实更新时修改并保留版本；用户要求删除、信息过期失效或触及隐私边界时删除，并通过审计记录验证结果。
...
> 来源：字节 Agent 开发实习一面 【小红书数据库智能化一面追问：主流 Agent 与 Claude Code 的上下文管理策略】【[阿里边缘bu 秋招一面 （已过）](https://www.nowcoder.com/feed/main/detail/bdebbb6088b6405e9eb2bd2c345acb6e)追问：你读过哪些 Claude Code 源码，了解它的上下文管理机制吗？】；[9.21 货拉拉二面](https://www.nowcoder.com/feed/main/detail/ec5dba791f4f457490c373194d2f20c2)
...
> 来源：快手 AI应用开发 一面；本轮追问：哪些场景适合滑动窗口，哪些场景更适合直接通过摘要进行上下文压缩？（[字节 Agent 秋招一面](https://www.nowcoder.com/discuss/929731481189044224)）
...
滑动窗口适合需要保留最近原文和精确指令的短期连续任务，便于追问和纠错；对超长、跨阶段且只需保留结论的历史，更适合摘要压缩。摘要可能遗漏否定条件和细节，关键事实应结构化保存，并可采用窗口加摘要的混合策略。
...
长任务先生成结构化 Plan，拆分目标、步骤、依赖、完成条件和失败回滚，并为每步记录状态。每次压缩后只保留当前计划、已完成动作、待办事项和关键约束；工具调用前校验其是否对应当前步骤，完成或失败后更新计划，禁止无计划的重复探索。
...
> 来源：字节跳动 Agent 二面（Coding Agent）；[乐言科技一面](https://www.nowcoder.com/feed/main/detail/70faa87a0e1046a68c864a2a7c4c6789)
...
可将 Claude Code 的压缩理解为接近上下文上限时触发的分层 compaction：保留系统规则、当前目标、未完成计划和关键变更证据，旧对话转为摘要。Tool Result 不应原样长期保留，可按字段提取、截断大输出、摘要并保存来源；需要时通过日志或工具重新获取，压缩后校验任务状态与依赖。
...
上下文接近上限时应预留输出和工具调用预算，按优先级裁剪闲聊、旧观察和可重取内容，保留任务约束、当前计划、最新错误及证据引用；同时将完整历史写入外部存储，必要时分阶段执行并在溢出后从 checkpoint 恢复，而不是直接截断。
...
工程上采用流式读取、分片落盘、有限队列和背压，处理完片段立即释放帧与中间张量，避免把多个视频同时载入内存。多个视频通常按 GPU、解码器和队列容量限并发；耗时取决于时长、采样率和模型，不能脱离这些条件给固定数值。若涉及生成视频，可按分镜生成后拼接，但需保留镜头顺序并处理转场、音频和跨镜头状态。
...
> 来源：月之暗面/Agent开发岗；本轮追问：是否把账号历史内容和图片统一交给模型总结？（[携程 AI 应用开发二面（已oc）](https://www.nowcoder.com/feed/main/detail/781cfc04e3bc4697acf0a5c913543a28)）
...
不应把账号全部历史文本和原图无差别交给模型。先按时间、主题和来源切分，文本做结构化摘要，图片通过视觉模型提取 caption、主体、风格等特征，并保留证据引用；再在统一预算内融合总结，必要时只检索相关片段，同时处理隐私和敏感内容。
...
> 来源：小红书 Agent 岗一面；本轮追问：那用户偏好怎么办的？（[本轮追问](https://www.nowcoder.com/feed/main/detail/51ff89e97d4949bd9db8e12a605b7aa9)）；本轮追问：博主文本和视觉风格蒸馏如何实现？（[携程 AI 应用开发二面（已oc）](https://www.nowcoder.com/feed/main/detail/781cfc04e3bc4697acf0a5c913543a28)）
...
蒸馏可采用多模态流水线：文本侧提取高频词、句式、叙事结构和语气，视觉侧用视觉模型提取构图、色调、主体与字体等特征，再合并为带证据的风格卡。生成时按任务检索相关风格约束，并用人工或相似度评估校验，避免把单篇内容误当成稳定偏好。
...
## Q：Work Memory 如何设计与更新？为什么能提升 Agent 的任务执行效果？

> 来源：[度小满一面](https://www.nowcoder.com/feed/main/detail/41baab8c631647568203ebfaf3898574)

**新手答**：“Work Memory 通常把当前任务的重要状态、计划、发现和失败经验结构化保存，并在后续步骤中按需检索，从而减少重复推理和上下文丢失。”

**高手答**：

Work Memory 是面向当前任务的短期、可更新状态层，不等同于把全部对话堆进上下文。可将目标、约束、已验证事实、待办、工具结果、失败原因和决策依据结构化存储，并为条目记录来源、时间、置信度和适用范围。Agent 在阶段切换或获得新证据时按策略写入、合并、修正或淘汰，再依据当前子任务检索相关内容；冲突信息应保留版本或回到来源核验。它有效的原因是减少重复观察和规划，降低长上下文噪声，并让执行状态跨步骤持续。工程上要限制写入膨胀、错误记忆、提示注入和敏感信息扩散，必要时支持人工覆盖。应通过有无 Memory 的消融实验，比较任务成功率、工具调用次数、延迟、成本和错误恢复率，而不是只看主观体验。

**差距在哪**：浅层回答只是保存历史摘要，深入回答要定义记忆结构、写入与冲突策略、检索边界，并用消融和成本指标验证是否真正提升执行效果。

---

## Q：大模型输出过长时如何进行长度控制与内容压缩？

> 来源：[9.14日美团多模态生成大模型二面](https://www.nowcoder.com/discuss/930520072370606080)

**新手答**：“通常会限制最大输出长度，要求模型先给结论、再压缩重复内容，必要时分段输出或让模型继续生成。”

**高手答**：

长度控制的目标不是简单截断，而是在预算内保留任务所需信息。可在请求层设置输出 token 上限、超时和停止词，并用结构化模板约束字段；生成后按优先级压缩，去掉重复背景，保留结论、证据、风险和待办。长文可采用“先摘要、按需展开”或分页续写，但续写必须携带稳定的摘要、已输出范围和幂等标识，避免重复或遗漏。应通过事实覆盖率、关键字段完整率、截断率、用户满意度和成本延迟评估策略；对代码、数字、引用等内容要使用专门校验，不能只依赖字数统计。主要权衡是压缩越强成本越低，但细节和可验证性可能下降。

**差距在哪**：浅层回答只会设置长度上限，深入回答还要设计分层压缩、续写状态、质量指标与失败边界，考查的是可控生成和评估能力。
