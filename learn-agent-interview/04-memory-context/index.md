...
新鲜度和重要性是记忆排序的两个独立维度，不能简单让一个压过另一个。处理思路是**双因子加权 + 衰减函数**：

```text
relevance_score = importance × decay(time_since_last_access)
```

- **重要性（importance）**：由记忆的语义权重决定——用户明确表达的偏好（“我对花生过敏”）权重高；闲聊、过渡性对话权重低。可以用分类器或规则标注
- **新鲜度（freshness）**：用时间衰减函数，比如指数衰减 `decay = e^(-λt)`，越久越低
...
**差距在哪**：新手用单一维度做排序。高手引入了双因子加权模型，且按记忆类型（事实/偏好/上下文）定义不同的衰减策略。面试官考的是你有没有把“排序”问题建模成一个可量化的工程方案。
...
Summarization 是**压缩**——把 10 段对话压成 1 段摘要，信息量减少但核心保留。Reflection 是**提炼 + 推理**——不只压缩，还要从记忆中**抽象出更高层次的认知**。
...
Reflection 的价值在于它产生了**原始记忆中不存在的新知识**——“正在学习 Python”“可以用 Java 类比”这些推断不是任何一条原始记忆直接包含的。

实现上，Reflection 通常是定期运行的后台任务：积累一批记忆后，让模型做一次反思性分析，产出高级认知，存入独立的“反思记忆层”。

**差距在哪**：新手把 Reflection 等同于 Summarization。高手用具体例子展示了本质区别——Summarization 压缩信息，Reflection 产生新认知。面试官考的是你对记忆系统“知识蒸馏”能力的理解。
...
| 精确键值查找 | KV 存储 | user_id → 最近 5 条对话 |
| 结构化条件过滤 | 关系数据库 | “最近 7 天内、重要性 > 0.8 的记忆” |
| 图关系查询 | 图数据库 | “和用户提到的'张三'相关的所有记忆” |

实际系统通常是**混合存储**：
- 用户画像和结构化偏好 → 关系数据库（PostgreSQL），支持精确查询和事务
- 最近对话和热记忆 → KV 存储（Redis），毫秒级读取
- 语义检索场景 → 向量数据库（Milvus），支持 embedding 相似度召回
- 实体关系 → 图数据库（Neo4j），支持多跳关联查询

向量数据库的局限：不擅长精确匹配、不支持事务、更新成本高。如果你的查询是“查用户的手机号”，向量数据库反而不如 KV 直查。

**差距在哪**：新手“全用向量数据库”是一刀切。高手按查询模式选存储，且指出实际系统是混合架构。面试官考的是你有没有数据存储选型的工程经验——不同查询模式对应不同的最优存储引擎。
...
工具态记忆是指 Agent 在调用工具过程中产生的**中间状态和执行上下文**，包括四类：

1. **工具调用历史**：哪些工具被调用过、参数是什么、返回了什么——避免重复调用
2. **工具状态快照**：外部系统的当前状态（如数据库查询结果、API 返回的实时数据），供后续步骤引用
3. **工具能力记忆**：某个工具在特定条件下的成功率、平均延迟、已知限制——辅助工具选择决策
4. **失败记忆**：哪些参数组合导致过工具调用失败——避免重复犯错
...
**差距在哪**：新手把工具态记忆理解成“记住工具说明书”。高手区分了四类工具态记忆（调用历史、状态快照、能力记忆、失败记忆），且说清了它在避免重复、上下文传递、动态选型三个场景的作用。面试官考的是你对 Agent 工作流中“执行态信息”管理的理解。
...
**差距在哪**：新手只做了简单乘法。高手从单条存储拆解、规模估算、成本优化、读写比分析四个层面做了完整规划，且给出了具体的数字。面试官考的是你有没有做过大规模存储系统的容量规划，能不能把一个抽象的“多大”变成具体的数字和优化策略。
