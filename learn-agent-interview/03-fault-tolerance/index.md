...
动态工具不能让模型直接执行：工具采用白名单、版本和参数Schema，模型只负责从候选工具中规划，权限校验、参数校验、重试和副作用提交由固定流程完成。财务数据以账务系统为唯一事实源，金额由代码计算并做对账，异常或高风险操作暂停人工确认。
...
> 来源：腾讯大模型应用开发二面 / [中兴软开一面](https://www.nowcoder.com/feed/main/detail/0b39815babfb47108464ffabdf929eba)；[智谱ai native builder实习一面](https://www.nowcoder.com/discuss/929419257438302208)
...
> 来源：腾讯 Agent 岗终面【[拼多多 - Agent 开发岗（工程化 + 数据库）](https://www.nowcoder.com/discuss/926273867092430848)追问：外部 API 超时的重试策略及防重试风暴？】；本轮追问：如果线上支付失败了，用你的系统排查，整个执行链路是怎样的？（[阳光电源  AI应用开发工程师 一面](https://www.nowcoder.com/feed/main/detail/122fd928ee824ed99c8f834233d1ac23)）
...
线上排查先用 trace_id 串起 Agent 决策、工具请求、支付订单、渠道响应和回调日志，区分“未发出、处理中、明确失败、结果未知”。对结果未知先查询订单或渠道状态并做对账补偿，所有重试带幂等键，确认最终状态后再向用户反馈。
...
终止还应包含目标完成校验：由规则或校验器验证结果，不满足才继续；工具失败按错误类型做有限次数、退避和幂等重试，插件钩子需记录并纳入预算。检测重复调用后可换工具或降级；高风险、反复失败或接近预算时暂停并转人工确认。
...
幻觉常来自训练数据噪声、知识过时、模型不确定性，以及检索证据缺失或冲突。多模态场景还会受图像文字识别错误、跨模态对齐不足和视觉细节缺失影响。可用带依据的 SFT/DPO 与难例训练降低倾向；RAG 同时检索文本、图片及 OCR 证据；再用结构化输出、规则校验、置信度阈值和人工审核拦截高风险结果。
...
不能仅靠模型拒答，应在数据访问层做分类分级与权限校验：按用户、租户、角色、字段和用途限制查询范围，默认最小字段与最小时间范围；批量导出需二次审批、脱敏和数量阈值，出口再做敏感信息检测，并记录请求、授权、结果摘要及审计日志。
...
大模型越狱通常指通过提示、对话或上下文操纵，绕过模型原有的安全对齐与使用限制；提示注入是实现这一目标的一类攻击手段，也可能诱导 Agent 泄露信息或执行越权操作。两者相关但不等价。
...
对车窗、车门等安全操作，执行前展示目标、动作和影响范围，要求用户明确确认；超时、指令歧义或语音误触则取消，必要时再由车端安全策略或人工审批复核。写操作按角色和资源范围授予临时权限；非代码项目修改先生成变更预览，只有获得用户对具体范围的授权后才执行，并记录授权、审批和结果。
...
座舱还要按驾驶员、乘客和访客区分身份与权限，麦克风、摄像头、位置及车辆状态遵循最小采集；能端侧处理则不上传，需上云时取得用户同意并脱敏。涉及车控的指令与隐私数据分离，断网、误识别或多人争抢时降级为只读和人工确认。
...
长视频应先按时间切片并行抽帧、ASR 转写和 OCR，分别检测画面、语音、字幕及音视频一致性，再按时间戳聚合风险片段。关键片段提高采样率或交人工复核，最终输出命中类型、时间范围和证据，避免只抽首尾帧漏检。
...
> 来源：蚂蚁 AI应用开发 二面 【淘宝闪购一面同题：人工强制中断 Agent 执行与 HiL 处理】；本轮追问：Agent 的读操作和写操作分别如何鉴权与控制风险？为什么写操作最终仍要跳转工单由用户确认？（[9.21 蚂蚁二面](https://www.nowcoder.com/feed/main/detail/52b419448b854e24bbdf41ca9e6ffe06)）
...
读操作也要按用户身份、资源归属和最小权限鉴权，并限制查询范围、频率及敏感字段；写操作除鉴权外还需校验参数、幂等性和风控策略。最终跳转工单并由用户确认，是为了让执行主体、目标和不可逆影响形成可审计的明确授权，避免模型误解、越权或被篡改参数后直接落地。
...
> 来源：慧疗互联网医院 Agent开发一面 【字节Agent开发实习生一面同题：“压缩方案是啥（三级压缩）”】【[格物致信（一面过，二面线下拒）](https://www.nowcoder.com/feed/main/detail/f68f0d54184944c391e0d7b6d1bb82c8)追问：Agent上下文窗口膨胀你是怎么解决的？难点是什么？】；[面完字节Agent，我人都傻了](https://www.nowcoder.com/discuss/929492609167290368)
...
超时要区分“未收到结果”和“工具可能仍在执行”：先设置端到端 deadline，超时后取消或查询任务状态；只有确认操作幂等且未完成时才按退避重试。不可确认状态的写操作不能盲目重试，应转人工或返回处理中，并记录超时原因。
...
并发上用队列、租户限流、优先级和背压控制模型及工具调用，任务状态机配合幂等键避免重复执行；运维关注队列积压、延迟、资源水位和SLO并设置告警、熔断与降级。归因按任务、租户、版本、工具和基础设施维度拆分，结合首个异常Span及重试后结果统计。
...
熔断切换不能只复制用户文本，应让各模型共享同一份有序会话状态，以消息版本或快照传递系统提示、历史消息、记忆和工具状态；切换前固定上下文版本，工具调用用幂等键防止重试产生重复副作用，完成后再提交新消息。
...
> 来源：[字节中国交易与广告 Agent 一面](https://www.nowcoder.com/feed/main/detail/6dede073825e4ab493fcbce7f598a6c8)（2026-08-24）；本轮追问：在内容审核方面应该使用什么大模型？（[本轮追问](https://www.nowcoder.com/discuss/926463586325495808)）；[Agent 开发面经](https://www.nowcoder.com/discuss/930572525178748928)
...
> 来源：小红书 Agent 岗一面【[启云方AI Agent一面凉经](https://www.nowcoder.com/feed/main/detail/fea2d18bd59a421da7d16fe16223d38c)追问：模型是不是返回一段普通文本，然后从文本里通过正则等方式解析出工具调用？】；[斑头雁（Agent实习生面试一面）](https://www.nowcoder.com/discuss/930139676764102656)
