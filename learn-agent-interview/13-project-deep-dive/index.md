...
情绪识别要单独定义标签和边界，使用覆盖不同语气、场景及否定表达的标注集，按用户或会话划分验证集；用混淆矩阵观察各类 Precision/Recall，并以低置信度转人工或追问兜底。上线后抽样复核和监控分布漂移，持续补充难例，而不是只看总体准确率。
...
可先解析出行日期、人数、预算和偏好，调用天气、交通、景点与酒店工具并行检索，再用计算工具核算总价和时间冲突；不满足预算或缺少信息时继续澄清，最终由规划器整合行程，预订类操作需用户确认后执行。
...
边缘 case 还包括：工具选择歧义或参数缺失、上下文过期、权限不足、超时与限流、返回空结果或部分成功，以及重复调用造成副作用。应设置超时、幂等键和重试边界，并记录请求与结果用于回放排查。
...
RAG 运行时先做向量与关键词混合召回，再按元数据过滤、重排，截取受上下文预算约束的片段交给生成模型，并返回来源。文档规模应分别说明单文档大小、总文档量和切片数量；若规模变化较大，按类型统计并通过检索耗时和召回效果验证容量设计。
...
算子优化可围绕性能瓶颈做：用 profiling 定位热点后进行算子融合，减少 kernel 启动和中间张量读写；通过布局转换、内存复用、批处理或异步流水隐藏搬运开销；对高频算子做缓存或专用实现，并以精度、形状兼容性和回归基准验证收益，异常时保留通用算子回退。
...
> 来源：淘宝闪购 AI应用研发 一面；本轮追问：如果现在用当下性能比较强的模型，你会从哪些角度去优化这个项目？（[苏州科达AI业务开 - 9月15日 - 一面 - 秋招](https://www.nowcoder.com/discuss/929790487429382144)）
