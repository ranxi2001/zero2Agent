...
> 来源：腾讯 Agent 应用开发一面；本轮追问：如果不使用框架，原生实现Agent你写过吗？使用框架的优劣势？（[美团AI Agent一面](https://www.nowcoder.com/feed/main/detail/50bcdc47e7754aa7be59b6318fea514b)）
...
原生实现通常是状态机或循环：维护对话状态，调用模型决定下一步，按 schema 分发工具，并补上超时、重试、权限、上下文裁剪和 Trace。优点是链路透明、可控、依赖少；缺点是要自行维护编排、异常恢复和工具协议。框架能提效和标准化，但也可能带来抽象泄漏、调试困难与版本耦合。
...
座舱应优先采用本地轻量模型处理唤醒、意图识别和高频指令，并为安全相关请求设置硬截止时间和无模型兜底。多 Agent 编排时先由编排器裁剪无关 Agent，共享上下文，按依赖将独立任务并行执行；支持首个可用结果提前返回、超时取消和降级，避免串行等待全部 Agent。
...
高并发并非简单由算法决定：令牌桶适合允许短时突发、保护吞吐，漏桶适合必须平滑下游流量。Redis 中为每个限流 key 保存 tokens 与 last_refill，Lua 内按 Redis 时间计算补充、截断到 capacity，再判断并扣减；脚本返回剩余量和重试等待时间，避免客户端并发竞态。
...
> 来源：快手 AI Agent 开发一面；[度小满AI全栈一面](https://www.nowcoder.com/discuss/931241025345978368)；[美团ai全栈一面](https://www.nowcoder.com/feed/main/detail/a5b8c6571f94441a8af10cd6af07ac3a)；[字节跳动Agent开发1面凉经](https://www.nowcoder.com/discuss/929406267141914624)
...
> 来源：快手 AI Agent 开发一面；本轮追问：你之前那一段实习的这样一个大数据导出是如何优化的？（[美团ai全栈一面](https://www.nowcoder.com/feed/main/detail/a5b8c6571f94441a8af10cd6af07ac3a)）
...
导出接口不应同步拼装全量结果：先创建异步任务，按游标或主键范围分批读取并流式写入分片文件，再合并或上传对象存储；记录任务进度、失败重试和幂等分片，完成后返回下载地址，并用压测校验内存、吞吐和耗时。
...
其底层是长连接 HTTP 响应：服务端返回 text/event-stream 后不结束响应，持续写入事件字节流；HTTP/1.1 常借助分块传输，HTTP/2 则复用流，浏览器按空行解析事件并触发回调。代理若缓冲响应会破坏实时性，通常需关闭缓冲并发送心跳保活。
...
> 来源：淘天 AI Agent 一面；本轮追问：超过单次模型时长限制后如何处理？（[携程 AI 应用开发二面（已oc）](https://www.nowcoder.com/feed/main/detail/781cfc04e3bc4697acf0a5c913543a28)）
...
> 来源：CVTE/AI应用工程师一面；本轮追问：如何让AI写业务代码？（[本轮追问](https://www.nowcoder.com/discuss/926539013991796736)）；[本轮来源](https://www.nowcoder.com/feed/main/detail/a11a3a9e0d824969b44db5bb2149ef9f)；本轮追问：AI Coding 有哪些局限性？（[携程 AI 应用开发一面（已oc）](https://www.nowcoder.com/feed/main/detail/2b1c35eace4a4bb3bd88ee669f163793)）
...
> 来源：bilibili AI研发实习一面；本轮追问：你是否实际使用过 LangChain 或 LangGraph？（[本轮追问](https://www.nowcoder.com/discuss/926528416512315392)）；本轮追问：简述主流Agent的核心组成架构。LangGraph在落地应用中的短板是什么？（[小红书多模态秋招二面](https://www.nowcoder.com/discuss/930755993066041344)）
...
主流 Agent 通常由模型、状态与记忆、规划/决策、工具调用、执行与观察、反馈循环及安全护栏组成。LangGraph 的短板是图和状态管理带来较高开发维护成本，流程复杂后可读性、调试、持久化和跨版本迁移都较难；应通过节点级日志、状态快照和故障注入验证链路。
...
> 来源：字节实习二面；本轮追问：如果没有 Redis，怎么设计缓存？使用什么数据结构？（[cvte应用软件开发一面](https://www.nowcoder.com/feed/main/detail/c2155d2308de452c8a6e3cf2bbb482f3)）
...
没有 Redis 时可用进程内有界缓存，以哈希表实现按键查找，配合双向链表实现 LRU，并用 TTL、最大容量和并发控制防止内存失控；以数据库或文件为事实源，接受重启丢失及多实例间短暂不一致。
...
> 来源：淘天 Agent 开发；本轮追问：为什么文档更新这里考虑使用 MQ，而不是直接在定时任务中处理？（[字节 Agent 秋招一面](https://www.nowcoder.com/discuss/929731481189044224)）
...
> 来源：Agent面经八股系列 / [蚂蚁 Agent 开发一面](https://www.nowcoder.com/feed/main/detail/39451cad5d2245b491d16778f2a9ca01)；本轮追问：你怎么做Agent的版本管理和OTA升级？（[蔚来——大模型算法岗（智能座舱/自动驾驶）实习一面](https://www.nowcoder.com/discuss/930755708008558592)）
...
OTA 不只是替换 Prompt：将 Prompt、工具 Schema、模型及运行时打成带依赖清单的可回滚发布包，签名校验后在设备或租户上分批下发；客户端先做兼容性检查并原子切换，持续上报下载、启动、失败和回滚状态，异常自动暂停灰度。
...
> 来源：牛客Agent面经汇总（含答题思路）；[字节 aime 一面 9.10](https://www.nowcoder.com/feed/main/detail/ed5e9d17f26e489da94afbf1241b885e)
...
不必强行共用一个模型：先按用户或内容区域、语言、数据处理边界和供应商可用性做硬路由，再在各区域候选模型中按质量、延迟和成本打分。两地保持统一接口与评测口径，并分别监控效果、失败率和漂移；只有离线评测与数据边界都满足时才合并模型池。
...
> 来源：点点互动/Agent开发秋招一面；[本轮来源](https://www.nowcoder.com/discuss/927594784770764800)；[cvte应用软件开发一面](https://www.nowcoder.com/feed/main/detail/c2155d2308de452c8a6e3cf2bbb482f3)
...
> 来源：小红书 Agent 岗一面；[本轮来源](https://www.nowcoder.com/feed/main/detail/8aa09d879bfc408fae7442565fd25fbe)；[字节数据中台Agent全栈面经](https://www.nowcoder.com/feed/main/detail/fe77496948f74b4091ab4bb6e8156c8b)；[度小满AI全栈一面](https://www.nowcoder.com/discuss/931241025345978368)
...
> 来源：北京 B 端 AI 小厂面经（2026-07）；本轮追问：几千个业务能统一 Agent 流程吗？怎么合并？（[本轮追问](https://www.nowcoder.com/feed/main/detail/a11a3a9e0d824969b44db5bb2149ef9f)）；本轮追问：如果让你对接企业知识问答Agent，飞书、钉钉、邮件怎么统一接入？（[百度Agent二面，不看简历不问八股](https://www.nowcoder.com/feed/main/detail/c7f00d0e48aa4017911b46ed928d15f3)）
...
对飞书、钉钉、邮件分别实现渠道 Adapter：统一解析 webhook/轮询事件、租户与用户身份、会话线程、附件和引用关系，再映射为 canonical message；发送侧按渠道调用对应 API 或 SMTP。邮件需处理 MIME、引用文本和附件，所有入口都做签名校验、幂等去重、重试及失败回执。
...
> 来源：深信服 AI 全栈开发二面；本轮追问：同一个 JVM 中，Agent 类的两个实例同时对共享计数器执行加一操作，并更新同一个共享集合，最终出现计数丢失，可能是什么原因？（[9.14小红书 PE（产品工程师/全栈方向--实习）二面 (流程泡到9.21挂)](https://www.nowcoder.com/discuss/929891805049421824)）
...
若两个实例实际引用同一个共享对象，计数器的“读取再加一”不是原子操作，会发生丢失更新；ArrayList、HashMap等非线程安全集合并发写入还可能导致数据覆盖、结构损坏或可见性问题。应改用并发集合，或用同一把锁保护完整复合操作，并验证共享引用、线程转储和并发测试结果。
...
## Q：如何保证 AI 生成内容的版权合规并避免侵权？

> 来源：[小红书多模态秋招二面](https://www.nowcoder.com/discuss/930755993066041344)

**新手答**：“不能保证绝对不侵权，只能通过使用合规数据、限制生成范围和人工审核来降低风险。”

**高手答**：

版权合规不是靠一句提示词或单次查重就能保证，而是端到端治理。先明确业务场景、输入输出边界和数据授权来源，建立数据许可、来源、版本与处理记录；检索增强时优先使用有明确授权的内容，并对输出做相似度、敏感片段和受限素材检测。对高风险用途设置拒答、引用溯源、人工复核和发布前审批，保留输入、模型版本、规则命中及处置日志。验证应覆盖已知作品、改写、拼接和长文本等测试集，并持续抽样审计；还要处理用户上传侵权内容、供应商条款和删除请求。无法做到绝对排除风险，需结合风险分级、降级和责任流程；以上是工程治理建议，不是法律意见。

**差距在哪**：浅层回答只会说查重和人工审核；面试官更关注数据授权、溯源检测、人工门禁、审计闭环及“无法绝对保证”的边界。

---

## Q：语音 Agent 如何处理噪声、用户打断与多轮对话？

> 来源：[蔚来——大模型算法岗（智能座舱/自动驾驶）实习一面](https://www.nowcoder.com/discuss/930755708008558592)

**新手答**：“通过降噪、语音活动检测和打断机制识别用户何时说话，再结合上下文维护多轮对话。”

**高手答**：

语音 Agent 通常拆成采集、回声消除/降噪、VAD、ASR、对话管理、TTS 和打断控制。噪声环境下应保留 ASR 置信度、端点检测和说话人状态，低置信度时确认或重问，而不是直接执行高风险操作；回声和双讲场景要避免把 TTS 当成用户输入。用户打断时立即停止或淡出 TTS，取消未完成任务，并用状态机区分“可打断播放、工具执行、不可撤销动作”。多轮上下文应按会话、任务和槽位管理，设置过期、压缩与敏感信息边界。验证要覆盖噪声类型、口音、重叠说话、网络抖动和连续打断，分别评估唤醒、端点、识别、响应时延和任务成功率；核心权衡是低延迟、低误触发与完整理解。

**差距在哪**：浅层回答停留在降噪和上下文；面试官会追问双讲、端点、可取消状态机、低置信度策略、敏感操作保护和分场景评估。

---

## Q：如何设计面向用户的 AI 服务用量配额、限流与容量规划？

> 来源：[cherrystudio面试](https://www.nowcoder.com/feed/main/detail/84ecd256fe4d4a1597a0f29f18f853fb)

**新手答**：“可以按用户设置每日调用次数或 token 上限，并通过限流和监控控制整体 AI 服务用量。”

**高手答**：

应先定义计量单位和策略：按请求、输入输出 token、模型成本或并发占用计费，并区分租户、用户、项目和接口。入口执行认证、配额预检查与限流，配合令牌桶或并发信号量；配额扣减要考虑流式响应、超时、重试和部分失败，采用幂等键或可补偿记录，避免重复扣减或绕过限制。容量规划应基于评测流量、峰值并发、上下文长度、模型延迟和供应商限额，设置保留容量、队列、降级及预算告警。通过压测、突发流量、故障注入和账单对账验证；严格限流会损害体验，过度预留则增加成本，还需防止多账号、代理转发和恶意提示造成资源滥用。

**差距在哪**：浅层只关注次数限制，深层要回答计量与扣减一致性、并发和容量模型、降级告警、滥用防护及体验成本权衡。
