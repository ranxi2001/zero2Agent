...
28. Tool Result 回写模型时，消息契约应该包含哪些字段？ — Newegg一面（新增）【字节火山引擎 Managed Agent 一面追问：Function Call 与 Tool Result 回到上下文】
29. MCP 返回结果支不支持流式？ — 淘天AI Agent一面（新增）
30. Tool-use SFT 的训练目标是什么？基座模型已经具备工具调用能力时，SFT 还需要学习什么？ — 唯品会NLP算法实习一面（新增）
31. 如何不用多智能体方案让 1000 个 Tools 正常工作？ — AI应用开发进阶面（新增）
32. 一个 Agent 如何同时连接多个 MCP Server，并保证用户与会话隔离？ — 百度秋招后端一面（新增）
33. CLI、Skill 与 sub-agent 的职责边界是什么？CLI 直接调用 LLM 和 Skill 拉起 sub-agent 应如何取舍？ — 小得盈满 AI 相关岗位一面（新增）
34. Agent 调用启动较慢的外部工具时，如何设计异步任务和结果回调？ — 途游 Agent二面（新增）
35. 跨平台工具授权即将过期时，Agent 如何调整调用顺序并安全续权？ — TikTok Agent工程师面试（新增）
36. MCP 工具治理为什么需要审计？应该审计哪些证据？ — 拓竹 AI Agent算法一面（新增）
...
12. 为什么在复杂的 Agent 闭环场景中，仅靠 RAG 无法彻底解决幻觉问题？ — 淘天一面 【淘天一面追问：数据/检索/生成三方面系统性降幻觉】【腾讯金融科技一面追问：知识库无内容但模型输出正确时的信任边界】
...
14. 什么是记忆的 Reflection 机制？ — 后端AI八股
15. 长期记忆选向量数据库还是 KV/关系数据库？ — 后端AI八股
16. 什么是记忆的幻觉问题？和 LLM 幻觉有何区别？ — 后端AI八股
17. 什么是“工具态记忆”（Tool-state Memory）？ — 后端AI八股
18. 记忆的容量规划需要考虑哪些因素？ — 后端AI八股
19. 如何判断当前对话与历史对话是否相关？ — 字节实习二面
20. 记忆摘要、压缩、去重、合并的触发时机？ — 30题 【淘天AI应用开发一面追问：向量记忆库去重方案与语义合并】
21. 一条历史信息该进长期记忆还是只留当前会话？ — 30题
22. 长期记忆检索时，怎么避免“语义相关但当前无用”的污染？ — 30题
23. 用户偏好、事实记忆、系统状态三者冲突了，信谁？ — 30题
24. Checkpoint 用什么数据库存？如何优化加载速度？ — AI工程师面试 【CVTE AI应用工程师一面追问：短期记忆为什么用 sqlite checkpointer】【钉学科技 FDE 实习一面追问：字段、一致性与换模型恢复】
25. Code Agent 的上下文工程，和普通对话 Agent 有哪些独特挑战？ — 蚂蚁一面
26. 做上下文工程最关键的工作是什么？ — 蚂蚁二面
27. 如何减少无关上下文对模型的干扰？当前上下文有哪些优化思路？ — 快手一面
28. 在电商或导购场景下，用户的请求往往高度模糊，Agent 怎么来精准理解这种需求？ — 淘天一面
...
40. 基于滑动窗口摘要时，合并还是分别保留？各自适合什么场景？ — 快手AI应用开发一面
41. 如果让你设计一个三层记忆机制，整体架构和压缩方法怎么设计？ — 快手AI应用开发一面
42. 你的向量记忆库是如何更新用户画像的？ — 快手AI应用开发算法一面
43. 压缩过程中会丢失工具调用历史，导致模型重复调用工具，怎么解决？ — 美团Agent开发（智能客服方向）二面
44. 记忆冲突怎么解决？比如用户前后说了不同的过敏信息 — 美团Agent开发（智能客服方向）二面
45. 短期记忆压缩后，过了很长时间又需要当时完整信息怎么办？ — AI初创Agent开发实习
46. 如何判断是 Prompt 内容影响决策，还是 Prompt 太长导致注意力涣散？ — 淘天AI Agent暑期实习一面
47. 为什么要区分静态长期记忆和动态长期记忆？各自存什么？ — 字节跳动Agent二面（Coding Agent）（新增）
48. 每轮对话都触发长期记忆存储，用户记忆快速积累、存得过多怎么办？ — 字节跳动Agent二面（Coding Agent）（新增）
49. 已进行 10 轮并做了总结，第 11 轮开始时，总结怎么处理？是重算前 11 轮还是叠加？ — 月之暗面Agent开发岗（新增）
50. 前 10 轮都变成了总结，之前的原始上下文就不需要了吗？ — 月之暗面Agent开发岗（新增）
51. 当用户对话零碎、跨轮次且意图发生跳跃时，如何结合上下文准确判断当前意图？ — 某小厂FOSHO AI应用开发二面（新增）
52. session 里的临时文件存主服务还是 skill 进程服务，要不要删，什么时候删？ — 阿里淘天Agent开发一面（新增）
53. Agent 做上下文压缩后，如何验证没有破坏当前任务？ — Coding Agent面经（新增）
54. 大体积工具结果落盘后，为什么还要返回预览？预览内容应该如何选择？ — 小红书 Agent 岗一面（新增）
55. 跨会话记忆如何从对话中提取？哪些信息值得写入长期记忆？ — 小红书 Agent 岗一面（新增）
56. 如何用 Prompt 提取用户风格偏好？风格偏好应包含哪些内容？ — 小红书 Agent 岗一面（新增）
57. 大模型生成会话摘要时，如何避免摘要内容污染用户偏好？新结论推翻旧结论时怎么保留？ — 百度内容营销与广告日常实习一面（新增）
58. 金融 Agent 执行股价提醒等定时任务时，应该携带哪些历史上下文？ — 顺极 Agent开发二面（新增）
59. 上下文预算不足时，如何按任务依赖压缩，而不是按时间删除旧消息？ — 腾讯互娱全栈开发（AI）二面（新增）
60. 云端 Coding Agent 的容器迁移或重启时，如何恢复会话上下文、工作区和进行中的任务？ — 腾讯 WXG微信读书一面（新增）
61. 按大纲分章节生成长文时，如何维持跨章节连续性与事实一致性？ — 成都 Agent 实习面经（新增）

## 05-eval-and-vision（41题）
...
19. 怎么理解 Vibe Coding？你有哪些实践经验？ — 蚂蚁AI应用开发二面 【科大讯飞一面追问：各Vibe Coding工具特点与CC/Codex使用感受对比】
20. 如何衡量 Agent 的 Planning 能力 vs Hallucination Rate？ — 淘天一面
21. 设计一个电商客服 Agent 的评测方案——商品咨询、售后处理、投诉安抚三类任务分别评估 — 淘天一面
...
25. Agent 后续的发展方向？哪些场景更容易落地？ — 视频面经汇总（新增）
...
31. Agent 自进化闭环如何设计？怎样判断沉淀出的经验值得进入系统？ — 字节Agent开发实习生一面（新增）【电商库存一面追问：人工审批、C 端灰度与回滚】【字节火山引擎 Managed Agent 一面追问：自动更新 AGENTS.md / Skills 后验证提升】
32. 如何证明 Agent 的最终答案真正使用了工具或检索证据，而不是凭模型常识猜中？ — Momenta Agent开发一面、阿里 Agent开发一面（新增）
33. Skill 路由应该如何构造测试集并评估？ — 字节Agent测评一面（新增）
34. Multi-Agent 出现 Badcase 时，如何定位责任 Agent，并判断是否需要 SFT？ — 字节Agent开发二面（新增）
35. Skill 的调用量、Token 成本和效果埋点应该放在哪一层？ — 电商库存二面（新增）
...
41. 树形意图识别和逐层路由应该如何设计，并构造评测集避免误差级联？ — 快手 AI应用开发一面（新增）
...
26. 多个 Agent 并行跑的时候状态竞争怎么避免？ — 淘天AI Agent一面（新增）
27. 如果拆成感知/推理/校验 Agent，哪些能并行哪些要顺序，什么时候需要反向通信？ — 商汤大模型算法应用实习二面（新增）
28. 校验 Agent 和推理 Agent 结论冲突时怎么处理？ — 商汤大模型算法应用实习二面（新增）
29. 在 A2A 场景下，如何防止两个 Agent 陷入递归对话？ — AI应用开发进阶面（新增）
30. 业务模块增删时，如何治理 Multi-Agent 能力拓扑，避免 Agent 增殖和路由配置失控？ — 懂车帝 Agent 开发一面（新增）
31. 如何按租户、任务和 Agent 层级设置分层并发预算？ — 小红书 AI Agent开发一面（新增）
32. 如何保证多 Agent 通信结果明确、可验证，而不是自然语言互相猜？ — 国际业务 Agent一面（新增）
33. 复杂 Agent 为什么拆成 LangGraph 子图而不是单条 Pipeline？子图的状态与 IO 契约如何设计？ — 小红书 Agent开发实习一面（新增）
34. 多 Agent 执行策略如何根据任务动态选择，并在运行中安全切换？ — 字节 AI Agent 二面实习面经（新增）

## 07-engineering-pitfalls（66题）


1. Agent 的成本怎么控制？ — 高频题 【字节实习二面追问：LobeChat 为什么烧 token】
...
17. 团队里的 Skill 数量持续膨胀，如何治理重复能力、路由冲突和上下文占用？ — 电商 Agent三面、电商库存二面（新增）
18. OpenSpec/Spec 驱动开发与普通开发流程有什么区别？如何治理 Spec 过期？ — 浦金科一面、电商库存一面（新增）
19. Skill 分层体系怎么设计？为什么这么分层？ — 字节跳动Agent二面（Coding Agent）（新增）
20. 动态 Prompt 和静态 Prompt 有什么区别？各自在什么场景下用？ — 字节跳动Agent二面（Coding Agent）（新增）
21. 如果让你设计一个代码审查的 Skill，你会如何设计？ — 最有料AI实习生面经（新增）
22. 如果 Agent 挂 100 个 Skill，如何提升召回率、准确度、F1 综合值？ — 关于skill的面试问题（新增）
23. 如何给 Agent 工具系统设计动态 Skill，而不让版本升级破坏历史任务？ — 关于skill的面试问题（新增）
24. Skill 和 Agent 的关系，为什么不用 Skill 而用子 Agent？ — AI应用开发进阶面（新增）
25. Skill 的 Prompt 配置上线后出错，如何快速止损和修复？ — 百度秋招后端一面（新增）
...
## 09-rag-retrieval（65题）

1. 多维度的查询改写是什么？ — 抖音一面 【淘天一面追问：改写为何提升精准度的底层原理】
2. RAG 的检索如何实现？ — 阿里一面
3. 并行化意图识别是什么？ — 抖音一面
4. 讲一下项目里召回的流程 — 抖音一面
...
33. Deep Research 在代码层面是怎么实现的？和普通 RAG 有什么区别？ — bilibili AI研发实习一面【字节火山引擎 Managed Agent 一面追问：Auto Research】
34. RAG 知识库的噪声剔除和文档去重怎么做？ — 腾讯AI应用开发二面 【淘天Agent开发追问：防止AI批量生成虚假数据投毒】
35. RAG 架构与模型微调（Fine-tuning）相比，各自的适用场景和优缺点是什么？ — 字节二面 【淘天转正实习一面追问：预训练语料已包含相关知识为什么还要RAG】
36. PDF 解析用什么工具？Layout-aware Parsing 是怎么做的？ — 腾讯AI应用开发二面
37. 图检索、向量检索、混合检索有什么区别？怎么选？ — 腾讯AI应用开发二面
38. 多模态 Embedding 检索中，文本与图片权重怎么平衡？图纸参数召回不准的根因？ — 淘天AI应用开发一面
39. 向量数据库的标量条件过滤怎么做？Pre-filter vs Post-filter 的区别？ — 淘天AI应用开发一面
40. Agentic RAG 是什么？和传统 RAG 的核心区别？ — 美团Keeta一面
41. 如何处理 RAG 过程中的权限隔离和时效性问题？ — 字节二面
42. 补充检索是如何评估数据质量并触发的？怎么保证二次检索能搜到之前没搜到的内容？ — 淘天Agent开发
...
53. Text2SQL 的 RAG 架构里，DDL 层和规则层分别解决什么问题？业务表频繁变更时怎么保持可用？ — 已有正文（补录索引）
54. 笔试题：多路召回结果合并去重 + 加权排序 + TopK — 已有正文（补录索引）
55. RAG 文档切分中遇到代码块、表格、标题等特殊内容怎么处理？ — 最有料AI实习生面经（新增）
56. 处理一万个长文档构建 RAG 知识库，工程上怎么做？ — 阿里Agent面经·场景题（新增）
57. RAG 知识库更新怎么不停服？热更新方案怎么设计？ — 腾讯AI应用开发（新增）
58. 混合检索到底在哪个环节比单独用效果好？ — 淘天AI Agent一面（新增）
59. MMR 为什么还能提高效果？重排后为什么还要设置 MMR 截断？ — 深势科技一面（新增）
60. 基于关键词的命令行代码搜索与基于 Embedding/RAG 的代码搜索，各有什么优缺点？ — 某小厂FOSHO AI应用开发二面（新增）
61. 知识库持续更新时，如何保证一次 RAG 回答读取同一逻辑快照？ — 腾讯互娱全栈开发（AI）二面（新增）
...
1. 构造数据集遇到过什么难点？ — 腾讯AI应用开发 【CVTE AI应用工程师一面追问：合成数据集质量达不到预期怎么办】
2. 预训练数据清洗方法？ — 字节一面
3. Agent 工具调用怎么训练？训练集包含什么？ — 腾讯二面
4. DPO、PPO、GRPO 的区别和优缺点？ — 高频题 【阿里国际一面追问：PPO vs GRPO 深度对比】【淘天AI应用开发一面追问：DPO不需要在线采样的原因+数据格式】【美团Keeta一面追问：chosen/rejected数据生成实操与常见坑】【阿里国际一面追问：重要性采样在策略差异大时失效+GRPO vs PPO KL散度区别】
5. 微调方法有哪些？LoRA 和全参数微调？ — 高频题 【阿里国际二面追问：微调经验与全流程认知】【淘天AI应用开发一面追问：LoRA A/B矩阵初始化+秩选择+权重Merge】
6. 给定时间序列，如何用 ML 筛选特征再基于规则建模？ — 字节一面
7. kernel 级别的优化，CUTE DSL 或手写 CUDA？ — 字节一面
8. 位置编码的作用？ — 高频题 【字节二面同题：QKV机制+为什么引入位置编码和多头注意力】
9. 绝对位置编码和相对位置编码的区别？ — 高频题 【爱奇艺大模型算法追问：RoPE/MRoPE长文本位置编码原理】
10. 手撕 Multi-Head Attention — 手撕题
11. 常用解决过拟合的方法？ — 高频题
12. LayerNorm 和 BatchNorm 的区别？ — 高频题
13. RLHF 中奖励模型训练数据如何构建？ — 后端AI八股 【快手一面+荣耀一面追问：奖励函数设计逻辑和打分规则】【阿里国际AI应用算法追问：reward hacking 识别与防范】【阿里国际AI算法一面追问：RL训练质量达标判断方法】
14. 大模型推理加速技术有哪些？ — 后端AI八股 【阿里国际二面追问：推理服务部署/算子融合/IO 优化】【阿里国际AI算法一面追问：Flash Attention与稀疏注意力原理】
15. 你了解哪些多模态大模型？ — 蚂蚁一面
16. 有没有了解过端侧部署的模型？ — 蚂蚁一面
17. 多模态是怎么实现的？图片怎么编码？ — 后端AI八股 【爱奇艺大模型算法追问：CLIP图文对齐+图像Token冗余解决方案】
18. 如何优化长文本生成中的显存占用？ — 后端AI八股
19. Transformer 中梯度消失/爆炸怎么解决？ — 后端AI八股
20. Token 过长导致的 Attention 稀释现象为什么会导致 Agent 的指令遵循能力下降？ — 淘天一面
21. 在 Agent 多轮对话任务中，标准 Attention 机制的平方复杂度在工程落地上主要引发了哪些问题？ — 淘天一面
22. SFT、蒸馏、GRPO 的技术选型——什么时候用什么？ — 阿里国际一面 【快手二面追问：SFT为什么不够，什么时候必须上RL】【腾讯金融科技一面追问：蒸馏时防止小模型学到错误推理链】
23. GRPO 的 Loss 函数、Advantages 计算与信用分配机制 — 阿里国际一面 【快手一面+美团一面追问：全0全1 reward处理 + 单步问题是否需要GRPO】【阿里国际一面追问：GRPO训练中观测什么指标】
24. 深度学习网络中的「残差连接」解决了什么问题？其物理含义是什么？ — 字节二面 【淘天Agent开发同题：传统CNN痛点+ResNet核心思想】
25. Text-to-SQL 训练集构建——500 样本覆盖 200+ 查询模式的数据生产流设计 — 淘天一面
26. DP、DDP、TP、PP——分布式训练并行策略的区别与选型 — 荣耀一面
27. Token 和字符有什么区别？ — 淘宝闪购一面
28. QLoRA 的核心设计是什么？NF4、双重量化、分页优化器分别解决什么问题？ — 淘天AI应用开发一面
29. 垂直领域微调后，模型通用能力严重退化怎么办？ — 淘天AI应用开发一面
30. 现有的大模型性能为什么这么好？ — 字节二面
31. vLLM 的 PagedAttention 原理是什么？解决了什么问题？ — 快手AI应用开发算法一面
32. GQA 和 MLA 的原理是什么？各自解决什么问题？ — 阿里国际AI算法一面
33. MoE 架构下为什么参数量大但单 Token 推理成本不一定高？ — 字节大模型测开一面
...
48. 训练后量化的完整流程是什么？粒度、校准方法和离群值如何共同影响精度？ — 摩尔线程/智谱/百度 AI Infra 面经（新增）
49. GPTQ、AWQ、SmoothQuant 与 AdaQuant 的核心思路有什么不同？ — 智谱/后摩智能/AI Infra 小厂面经（新增）
50. Tool-use SFT 训练时，长轨迹采用截断、切分还是掩码？如何避免破坏工具依赖关系？ — 唯品会NLP算法实习一面（新增）
51. 为什么 Step-level SFT 之后再进行 GRPO，通常比直接从基座模型开始做 GRPO 稳定？ — 唯品会NLP算法实习一面（新增）
52. GRPO 中相对奖励是如何计算的？同一组奖励方差接近零时如何处理？ — 唯品会NLP算法实习一面（新增）
53. Tool-use 轨迹长度与任务复杂度有什么关系？训练数据应如何分布？ — 唯品会大模型算法实习一面（新增）
54. Tool-use 强化学习中的内容奖励应如何设计？ — 唯品会大模型算法实习一面（新增）
55. 多工具调用存在依赖关系时，Reward 应如何做信用分配？ — 唯品会大模型算法实习一面（新增）
56. Agent 交互轨迹与普通语言模型语料有什么区别？如何仿真高质量轨迹？ — 字节Agent算法实习一面（新增）
57. Agentic CPT、SFT、RL 三阶段分别训练什么能力？ — 字节跳动AI Agent秋招一面（新增）
58. 多轮对话 RL 如何设计过程奖励与终局奖励，并避免用户模拟器过拟合？ — 阿里云 AI Infer一面（新增）
59. 为什么 SFT 后继续做 DPO/PPO 等偏好优化可能导致基础能力退化？ — 哔哩哔哩 AI 后端开发凉经（新增）
60. LoRA 应该挂在哪些层？rank、alpha 和 dropout 如何共同影响效果？ — Shopee 大模型一面（新增）
61. KV Cache Block 的哈希和逻辑到物理映射应该怎么设计？ — 智象未来 AI Infra一面（新增）
62. Agent 动作空间过大导致探索低效时，如何裁剪和分层？ — 阿里千问 C端算法实习一面（新增）
63. Agentic RL 与普通 LLM RL 的核心差异是什么？ — 腾讯大模型算法岗一二面（新增）
64. Agentic CFT 与 SFT、RL 的目标有何不同？为什么训练时要 Mask Observation Token？ — Shopee Agent开发一面（新增）
65. 训练实验如何对 YAML 配置做规范化哈希，并保证单变量变化可复现？ — 大方云图研发实习一面（新增）
66. 如何训练模型做高精度抽取式摘要？数据、目标、Loss 和评测如何设计？ — 百度大模型实习 Agent 面经（新增）
...
8. Coding Agent 如何执行人工交互测试，例如操作浏览器并验证页面行为？ — 小红书 Agent 岗一面（新增）
...
13. 如何用 Agent 自动化测试一个现有软件项目，并划分规划、执行、Oracle 与人工门禁？ — 蚂蚁集团效能研发面经（新增）

## 12-business-ai-engineering（17题）
...
7. 设计订单客服 Agent 时，如何处理转人工和意图识别调优？ — 某 Java/Agent 岗面经（新增）
8. Agent 项目如何从 Demo 进行企业级落地？从原型到生产需要补全哪些工程能力？ — 哆咔互娱Agent开发实习一面
...
3. 如何从模型结构估算参数量、FLOPs、训练显存、推理访存与 MFU？ — 美团/混元/讯飞/字节等 AI Infra 面经（新增）
4. Prefill 与 Decode 的算子形态和瓶颈为何不同？ — 小马智行/阿里云/腾讯/爱奇艺 AI Infra 面经（新增）
5. 如何估算 All-Reduce/All-to-All 通信量并实现计算通信重叠？ — 阶跃星辰/快手/字节/爱奇艺等 AI Infra 面经（新增）
6. MoE 的 Expert Parallel 如何做 Dispatch/Combine、负载均衡和通信优化？ — 阿里/美团/快手/字节等 AI Infra 面经（新增）
