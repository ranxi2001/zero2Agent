...
def llm_extract_questions(
    ...
    system_prompt = (
        ...
        "排除职业规划、Offer/转正、薪资职级、工作地点、离职原因、兴趣爱好、反问，以及作者信息、评论、推荐、答案、岗位职责和面试感想。"
        "项目追问只有在保留具体技术对象或工程约束时才提取，不输出‘介绍项目’‘实习做了什么’等泛问。返回严格 JSON："
...
def llm_classify_interview_experience(
    ...
    system_prompt = (
        "判断输入是否为作者本人亲历的一条连贯技术面试记录。只有作者明确参加了该公司/岗位的技术面试，并记录同一面试流程或连续轮次中的具体技术问题时才为 true；纯 HR 面、电话筛选和只有个人背景问题的记录为 false。"
        "把多个无关日期、公司或岗位拼在一起的合集，即使有详细题目也为 false；带标准答案、固定项目指标、剩余百分比、购买专栏或内容账号模板的重构稿为 false。"
        "招聘、内推、Offer选择、求助、学习路线、教程、题库整理、纯笔试、付费推广、评论转载、相关推荐和热榜均为 false。"
        "返回严格 JSON："
