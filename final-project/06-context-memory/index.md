...
```text
┌──────────────────────────────────────────────┐
│ System Prompt（固定层）                        │  ≤ 2000 tokens
│ 诊断标准、评分维度、输出格式约束               │
├──────────────────────────────────────────────┤
│ User Profile（半固定层）                      │  ≤ 500 tokens
│ 用户背景、目标岗位、历史弱点                   │
├──────────────────────────────────────────────┤
│ Task Context（动态层）                        │  ≤ 4000 tokens
│ 当前题目的 Q&A + 知识库参考                    │
├──────────────────────────────────────────────┤
│ History Summary（压缩层）                     │  ≤ 2000 tokens
│ 已完成题目的诊断摘要                           │
├──────────────────────────────────────────────┤
│ Recent Messages（滑动窗口）                   │  ≤ 3000 tokens
│ 最近 3 轮完整对话                              │
├──────────────────────────────────────────────┤
│ 输出空间                                      │  ≥ 4000 tokens
│ 留给模型回复的 token 余量                      │
└──────────────────────────────────────────────┘
总窗口: ~16000 tokens（Sonnet 可用 200k，但控制成本和注意力质量）
```
...
- [07-permission-session：权限系统与会话恢复](../07-permission-session/index.html)（待产出）
